...
def main():
    ...
    input_size, input_channels, n_classes, train_data , val_dat,test_dat = utils.get_data(
        config.dataset, config.data_path,config.val1_data_path,config.val2_data_path, cutout_length=0, validation=True,validation2 = True,img_size = config.img_size)
    ...
    #load
    if(config.load):
        model,config.epochs,optimizer,criterion = utils.load_checkpoint2(model,config.epochs,optimizer,criterion,config.check_path)
    ...
    for epoch in range(config.epochs):
        ...
        if(config.save):
            utils.save_checkpoint2(model,epoch,optimizer,criterion, config.check_path, is_best)
        print("")
...
def testing(valid_loader, model, criterion,epoch, cur_step,overall = False):
    top1 = utils.AverageMeter()
    top5 = utils.AverageMeter()
    losses = utils.AverageMeter()
    model.eval()
